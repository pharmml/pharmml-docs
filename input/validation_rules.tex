...
\valrule{S8}{Elements, which are explicitly typed must be associated with quantities of same type} Quantities
...
\begin{valrules}
  \valrule{S13}{A column reference must always to resolve to a
  ...
  dataset.
\valrule{S14}{A mapping between a symbol or object to a column in a
...
\begin{valrules}
\valrule{DS1}{All columns must be present in the file.}
...
(CSV) format.
\valrule{DS6}{Alowed set of delimiters} Delimiters allowed are COMMA, SEMICOLON, SPACE 
