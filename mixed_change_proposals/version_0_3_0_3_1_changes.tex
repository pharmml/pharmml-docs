...
\lstdefinelanguage{XML}
{
  ...
  math,ct,ds,mdef,mstep,mml,un,name,definition,writtenVersion,id,inputType,oidRef,catId} % list your attributes here
}
...
This document describes changes and extensions in \pml
\begin{itemize}
\item
between versions 0.2.1 and 0.3  -- chapter 2
\item
and between versions 0.3 and 0.3.1 -- chapter 3.
\end{itemize}
It is not intended to serve as full specification, such the one for the frist public \pml release \cite{Pharmml_021}. 
However, the next public release, due in M42, will come with a fully updated specification document.
The current document is based on the feedback we have received during the DDMoRe-internal \pml workshop in 
Pavia (November 2013), from WP1 modellers, the MDL-\pml task-force group discussions, the exchange with the 
\pml-NMTRAN translation team and via other communication channels. 

Following tables summarise the changes described in detail in following chapters.


\paragraph{Changes between 0.2.1 and 0.3}

\begin{center}
\begin{longtable}{lll}
  \hline
  \hline
\pml element 	&  version 0.2.1 			& version 0.3 \\
or modelling aspect 	&						& \\
  \hline
...
\paragraph{Changes between 0.3 and 0.3.1}

\begin{center}
\begin{longtable}{lll}
  \hline
  \hline
\pml element 			&  version 0.3 						& version 0.3.1 \\
or modelling aspect 		&								& \\
  \hline
Mapping multiple  		& \emph{not supported}				& \xelem{MultipleDVMapping} element \\
dependent variables 	& 								&  {\color{red} \scshape{new}} \\ 
(see \textsection\ref{sec:multipleDVs})						&	& \\
\hline
Dosing mapping		& \emph{not supported}				& -- \xatt{cmt} attribute {\color{red} \scshape{new}} \\ 
from NONMEM datasets	&								& -- \xatt{compartmentNo} attribute \\
(see \textsection\ref{sec:columnODEmapping})				&	&  {\color{red} \scshape{new}} \\
\hline
File referencing 		& using \xelem{filename} and \xelem{url} 	& -- \xelem{path} element instead {\color{red} \scshape{new}}\\
(see \textsection\ref{sec:simplifiedReferencing})	 			&	& -- \xatt{SEMICOLON} delimiter {\color{red} \scshape{new}}\\
\hline
Referencing NONMEM 	& using \xelem{TargetToolReference}	& \xelem{NONMEMdataSetReference} \\
datasets (see \textsection\ref{sec:NONMEMdataSetReference})	&	& {\color{red} \scshape{new}} \\
\hline
Simulation support 		& -- only explicitly via \xelem{TrialDesign} & -- \xelem{Observations} optional \\
(see \textsection\ref{sec:improvedSimulation}) 				& -- \xelem{Observations} element		& -- NONMEM datasets can  \\
					& mandatory 						& be used \\
\hline
Referencing categorical	& possible using strings only			& extended to numerical \\
covariates	 (see \textsection\ref{sec:referencingCategories}) 		&	& identifiers \\
  \hline
\caption{Overview of major differences between versions 0.3 and 0.3.1}
\label{figTable:overviewTable}
\end{longtable}
\end{center}
...
\end{lstlisting}
This has disadvantages such as 
\begin{itemize}
\item
file size of XML based datasets increases by factor 5-10 compared to plain format files. Plain datafile
sizes in the region GB are possible, making the XML version difficult to process.
\item
it requires provision of additional translation tools to convert standard datasets to this format
\item
editing such data sets by hand is very error prone.
\end{itemize} 
Therefore externalisation of datasets is supported in the 0.3 release. As shown in the 
following listing, the external data set can be specified alternatively in the new 
\xelem{ImportData} element, instead of \xelem{Table}, by providing the file name, 
URL, file format and delimiter. Only one file format is allowed so far, the 
...
in three dedicated datasets section within \pml, more specifically as 
...
Using NONMEM datasets has consequences on the content of the \pml document in that
the \xelem{TrialDesign} part is in such case redundant. Table \ref{tab:withAndWithoutNM} below 
...
\begin{tabular}{ll}
  \hline
 Using \xelem{TrialDesign} & Using NONMEM format dataset \\
  \hline
  \lstset{language=XML}
\begin{lstlisting}
<ModelDefinition>
...
\\
    \hline
\end{tabular}
\caption{Comparison of model structures encoded using \xelem{TrialDesign} (left) versus models using 
NONMEM format dataset (right). Note that in the latter case the \xelem{TrialDesign} part redundant.}
\label{tab:withAndWithoutNM}
\end{center}
\end{table}


We start with the description of the column mapping of the NONMEM data file to the according elements in the 
\xelem{ModelDefinition}. Then the dataset reference will be discussed and at the end we will look 
how to handle NMTRAN code injection. The warfarin dataset is used 
for the first two sections, \emph{warfarin\_conc\_pca.csv}, see Table \ref{tab:warfarinData}.
...
1	& 1	& 	66.7		& 50	& 	1	& 	.	& 	1	& 	1.9	& 	0 \\
1	& 2	& 	66.7	& 	50	& 	1	& 	.	& 	1	& 	3.3	& 	0 \\
1	& 3	& 	66.7	& 	50	& 	1	& 	.	& 	1	& 	6.6	& 	0 \\
... & ... & ... & ... & ... & ... & ... & ... & ... \\
2	&	0	& 66.7	& 	50	& 	1	& 	100	& 	0	& 	.	& 	1 \\
2	&	0	& 66.7	& 	50	& 	1	& 	.	& 	2	& 	100	& 	0 \\
2	&	24	& 66.7	& 	50	& 	1	& 	.	& 	1	& 	9.2	& 	0 \\
2	&	24	& 66.7	& 	50	& 	1	& 	.	& 	2	& 	49	& 	0 \\
2	&	36	& 66.7	& 	50	& 	1	& 	.	& 	1	& 	8.5	& 	0 \\
... & ... & ... & ... & ... & ... & ... & ... & ... \\
  \hline
\end{tabular}
\caption{Snippet from the warfarin data set.}
\label{tab:warfarinData}
\end{center}
\end{table}
The Chan et al. dataset will be used in the last section to explain the code injection (Sec. \ref{subset:partIII}).
...
\item
Support for NMTRAN code injection (with symbol mapping)
\end{itemize}
...
\begin{lstlisting}
                <ColumnMapping>
                    ...
                    <ct:SymbRef blkIdRef="BLOCK_WHERE_SYMBOL_IS_DEFINED" symbIdRef="SYMBOL_NAME_IN_THE_MODEL"/>
                </ColumnMapping>
\end{lstlisting}
The following code (\pml file: warfarin\_PK\_PRED.xml) illustrates the mapping of dataset columns 
...
explained in the Sec. \ref{sec:modellingSteps}.
\lstset{language=XML}
\begin{lstlisting}
    <!-- MODELLING STEPS -->
...
\pml allows only one value for each type.

\begin{table}[ht]
\begin{center}
\begin{tabular}{lll}
  \hline
\xatt{use}/MDL & \xatt{columnType}/\pml & Meaning \\  
  \hline
...
time &  time  & time     \\
   \hline
\end{tabular}
\caption{A summary of allowed values for the \xatt{use} attribute in MDL and their 
counterpart \xatt{columnType} in \pml.}
\label{tab:MDLPharmML_columnTypes}
\end{center}
\end{table}

Additionally there are few \pml specific types the user can choose from to assign 
...
study design is defined explicitly in \pml without using the NONMEM dataset. Accordingly 
...
ssPeriod &     steady-state administration interval \\
  \hline
\end{tabular}
\caption{A list of additional values allowed for the attribute \xatt{columnType} in \pml.}
...
The \xelem{Definition} element is where all the column are specified. They are 
...
\end{lstlisting}
Once the definition is completed, we need to add the reference to the external file holding the data using 
\xelem{ImportData}, as explained in Section \ref{sec:externalFiles}.
...
This NMTRAN code is part of MDL. However, we suggest that it needs to be 
marked as target code and handled as such by MDL so that the translator knows 
which part is NONMEM-specific and is suppose to be passed without any interpretation.

Mapping of dataset columns and referencing of the NONMEM file, are similar 
to the previous section. Only symbols used in the \xelem{ModelDefinition} will be 
...
\end{lstlisting}
The following describes how the code injection works. The first section of the next listing shows 
...
between them -- unless a translator from NMTRAN/MDL to \pml will provide 
this information.)
...
It is still unclear whether mappings of e.g. R1, D1 are required. This question has to be addressed
during the MDL-\pml task-force meetings. One solution would be to extract 
all symbols out of the target code. Alternatively, MDL should deal with it by making sure that 
the target code passed to \pml follows well defined rules and grammar.

%\subsection{Suggested improvements}
%There was a discussion on how to improve this part of \pml at the last consortium 
...
\end{align}
\pml in version 0.2.1 provided the possibility to define pair-wise covariance or 
...
The variability level in the \pml version is called \emph{indiv} and the row names refer to 
...
This example shows the lower level of inter-occasion variability, encoded in 
NMTRAN/MDL as in the following table. The \pml implementation is very similar to 
the previous example, section \ref{subsec:example1} with the exception that the 
\xelem{VariabilityReference} contains a reference to \xatt{iov1} and the \xelem{RowNames} are different.
...
\end{lstlisting}
The rest is identical to the previous example, section \ref{subsec:example1}, and therefore omitted.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{General target tool support}
\label{sec:targetToolSupport}
\pml 0.3 supports data sets or scripts coming from other sources 
then NONMEM-format files, such as SIMCYP project files or R scripts.
...
This section illustrates how estimation/simulation steps can be applied. Let's consider 
that one would like to encode a simple workflow consisting of estimation and simulation tasks. 
The first based on the NONMEM format dataset, the second using user defined R-script. 
The following code visualises the implementation.
...
\pml covers with \xelem{TrialDesign} a wide range of dosing scenarios which can be 
used to encode virtually any PK model. Sometimes, the concentration data we want
to couple with a PD model is available as a lookup table, i.e. measurement records for which 
the underlying PK model is unknown or not essential. 

Another example common in diabetes modelling is when using the so called \emph{minimal model}.
It requires insulin input as discrete measurements. Also here the data is coming in form 
of a \emph{lookup table}, usually with two columns, one for time and the other for the 
dependent variable. Version 0.3 of \pml support this form of input and the next section 
will describe few typical scenarios.
...
a subsequent PD model using an interpolation method
\begin{itemize}
\item
case \emph{C1} -- the user can choose from a list of known algorithms, e.g. \{constant, nearest, linear, spline, chip, cubic\}\footnote{The 
choice is based on the interpolation algorithms provided by MATLAB, section 
\emph{interp1:1-D data interpolation (table lookup)}'  \url{http://www.mathworks.co.uk/help/matlab/ref/interp1.html}. 
Additionally the \emph{constant} algorithm type is supported.}
\item
case \emph{C2} -- the interpolation is an arbitrary user-defined piece-wise function
\item
case \emph{C3} -- no data is available but the parameters of a user-defined function are estimated.
\end{itemize}
...
\begin{tabular}{ll | ll | l}
  \hline
Case & Setup/Input  & TrialDesign & Target/ModelDefinition & Comments \\ 
  \hline
  \hline
  & \multicolumn{4}{ c }{\textbf{General time-varying input}} \\
  \hline
C1 	& Experimental data \&  	& -- Data as lookup table 		& -- Structural model 			& No \emph{Input}  \\ [-.25ex]
	& Interpolation type 		& -- Reference to \emph{Input}	& with \emph{Input}				& parameters \\ [-.25ex]	
	& from list	 			&						& -- Interpolation type,			& to be estimated \\ [-.25ex]
	& 					& 						&  e.g. \emph{constant}, \emph{linear} & \\ [1ex]
C2	& Experimental data \& 	& -- Data as lookup table 		& -- Structural model  			& No \emph{Input}  \\ [-.25ex] 
	& User-defined input		& -- Reference to \emph{Input}	& with \emph{Input} 				& parameters \\ [-.25ex]
	& function			 	& 						& -- \emph{Input} as piece-wise	& to be estimated \\ [-.25ex] 
	&   					&  						& function  					& \\ [-.25ex]
	& 					&  						&  							& \\  [1ex]
C3	& Data unavailable 		& 						& -- Structural model  			& Estimation of   \\  [-.25ex] 
	& User-defined input 	& 						& with \emph{Input} 				& \emph{Input} \\ [-.25ex] 
	& function			  	& 						& -- \emph{Input} as piece-wise  	& parameters is \\  [-.25ex]
	&					&						& function  					& of interest \\  [-.25ex]
...
\end{align}
implemented as shown in the following snippet:
\lstset{language=XML}
\begin{lstlisting}
...
Nested tables were used to store time-varying \emph{covariates} data in the 
\xelem{Population} element.

As described in section \ref{sec:externalFiles} external data files, in CSV format with three 
different delimiters, are supported in \pml 0.3.
...
\begin{lstlisting}
            <ObjectiveDataSet>
                ...
                <DataSet xmlns="http://www.pharmml.org/2013/08/Dataset">
                    ...
                    <!-- omitted data table -->
\end{lstlisting}                    
The listing below shows the 0.3 version of the mapping with the newly introduced element \xelem{ColumnMapping}
...
would be omitted. This lead to misunderstandings which can be avoided with the 
...
Previous versions of \pml assumed the initial time, $t_0$, to be always equal to $0$. 
This restriction is now relaxed and \pml allows to store any initial time. The initial value 
...
else in the model or arbitrary mathematical expression. See Tables \ref{tab:ICsupportMaths} 
and \ref{tab:ICsupport} for the changes in mathematical notation and \pml implementation, 
respectively.
...
\begin{tabular}{ccc}
  \hline
feature /\ version	& \pml 0.2.1 & \pml 0.3 \\ [1ex]
  \hline
ODE	& $\frac{dA}{dt} = -k_a Ad$ 	& $\frac{dA}{dt} = -k_a Ad$ \\ [1ex]
IC 	&	$A_0$				&	$A(t_0=a) = A_0$ \\
  \hline
\end{tabular}
\label{tab:ICsupportMaths}
\caption{Summary of \emph{initial conditions}, IC, in ODE support in \pml. Version 0.2.1 
supported the implementation of initial values only, version 0.3 supports additionally the 
implementation of initial times $t_0$.}
\end{center}
\end{table}
...
\begin{tabular}{ll}
  \hline
  \pml version 0.2.1 & \pml version 0.3 \\
  \hline
 \lstset{language=XML}
\begin{lstlisting}
...
\end{lstlisting}  
\caption{Comparison of the encoding of initial conditions for ODEs. Version 0.3
allows to encode explicitly initial time and value, whereas version 0.2.1 allowed to 
specify the latter, assuming the default initial time equal 0.}
\label{tab:ICsupport}
\end{tabular}
\end{center}
\end{table}
...
\end{lstlisting}
\end{itemize}
During the recent \pml workshop an extended input definition was requested 
...
\chapter{Changes in \pml 0.3.1}
The following modifications and extensions have been requested during the encoding of the 
models intended to be used in the teaching courses in July.

\section{Mapping dependent variables}
\label{sec:multipleDVs}

\subsection{Standard case -- single DV}
Usually there is only one dependent variable in the \emph{DV} column of a typical NONMEM datafile.
In PhamrML, as explained in Section \ref{subset:partI}, such mapping is then done using the \xelem{ColumnMapping}. 
The following code snippet shows how the dependent variable is mapped with the \emph{Y\_obs} variable
in the \xelem{ObservationModel} named \xatt{om1}
 \lstset{language=XML}
\begin{lstlisting}
    <ColumnMapping>
        <ds:ColumnRef columnIdRef="DV"/>
...
\subsection{Multiple DVs}
Sometimes however, the situation is more complicated and the dataset contains multiple observations 
in one and the same DV column and one needs additional column, here ORIG to 
point the target tool to correct value. In the artificial dataset shown below three different DV's are stored and the 
column ORIG carries the number to identify each value
\begin{table}[H]
\begin{center}
\scriptsize
\begin{tabular}{cccc}
...
... & ... 	& ... 	& ... \\
  \hline
\end{tabular}
\caption{Multiple dependent variables in one DV column of the dataset. Additional ORIG column
is used to distinguish between 3 dependent variables encoded in column DV.}
%\label{tab:lookupData}
\end{center}
\end{table}

To be able to map multiple DV's to the proper symbol/variable in the model, a new element
\xelem{MultipleDVMapping} has been defined. It works similar to the \xelem{ColumnMapping}
tag in that its first child element refers to the appropriate dataset column. The second
child element refers as before to the target variable but this time using a piecewise statement 
able to condition the mapping based on the value of the ORIG column.

For instance, in the following dataset column mapping as in the \emph{Hamren\_CPY\_2008.xml} model
\begin{itemize}
\item
the first DV variable is being mapped to the \xatt{FPG\_obs} in the observation model \xatt{obs\_FPG}
\item
the second DV variable is being mapped to the \xatt{HBA1C\_obs} in the observation model \xatt{obs\_HBA1C}
\item
the third DV variable is being mapped to the \xatt{HB\_obs} in the observation model \xatt{obs\_HB}
\end{itemize}
...
\begin{lstlisting}
      <MultipleDVMapping>
        <ds:ColumnRef columnIdRef="DV"/>
        <!-- DV is mapped to 'FPG_obs' in observation model 'obs_FPG' if ORIG=1 -->
        <Piecewise>
          <Piece xmlns="http://www.pharmml.org/2013/03/Maths">
            <ct:SymbRef blkIdRef="obs_FPG" symbIdRef="FPG_obs"/>
            <Condition>
              <LogicBinop op="eq">
          ...
          <!-- DV is mapped to HBA1C_obs in observation model 'obs_HBA1C' if ORIG=2 -->
          <Piece xmlns="http://www.pharmml.org/2013/03/Maths">
            <ct:SymbRef blkIdRef="obs_HBA1C" symbIdRef="HBA1C_obs"/>
            <Condition>
              <LogicBinop op="eq">
                <ds:ColumnRef columnIdRef="ORIG"/>
                <ct:Int>2</ct:Int>
          ...
          <!-- DV is mapped to HB_obs in observation model 'obs_HB' if ORIG=3 -->
          <Piece xmlns="http://www.pharmml.org/2013/03/Maths">
            <ct:SymbRef blkIdRef="obs_HB" symbIdRef="HB_obs"/>
            <Condition>
              <LogicBinop op="eq">
                <ds:ColumnRef columnIdRef="ORIG"/>
                <ct:Int>3</ct:Int>
...
\section{Mapping between dosing column and ODEs}
\label{sec:columnODEmapping}
In Monolix and NONMEM the differential equations are implicitly numbered 
by the order in which they are encoded. \pml, where the order in general doesn't matter, needs 
a mechanism by which the mapping can be realised. Two extensions are necessary for this to work
\begin{itemize}
\item
\xatt{columnType} attribute gets an additional value \xatt{cmt}, corresponding to the value \xatt{CMT} 
of the \xatt{use} attribute in MDL
\item
new \xatt{compartmentNo} attribute has been defined for a \xelem{DerivativeVariable}. 
\end{itemize}
In practice, \xatt{cmt} attribute is assigned to the NONMEM dataset column such as CMT where 
the identifier of the dosing compartment is stored. Thanks to the \xatt{compartmentNo} the mapping is
then well-defined. 
...
time &  time  & time     \\
   \hline
\end{tabular}
\caption{An updated table of allowed values for the \xatt{use} attribute in MDL and their 
counterpart \xatt{columnType} in \pml. Compared to Table \ref{tab:MDLPharmML_columnTypes}
the \xatt{cmt} has been added.}
\label{tab:MDLPharmML_columnTypes_update}
\end{center}
\end{table}


\section{Simplified file referencing and new delimiter}
\label{sec:simplifiedReferencing}
The changes aim to simplify the notation and usage of referencing of external files from 
\pml and can be summarised as follows
\begin{itemize}
\item
Merge \xelem{name} and \xelem{url} elements to \xelem{path}
\item
\xelem{path} 
\begin{itemize}
\item
holds the file name and its extension
\item
is relative to the \pml file
\end{itemize}
\item
\xelem{delimiter} gets another value \xatt{SEMICOLON}, additionally to \xatt{COMMA, SPACE} and \xatt{TAB}.
\end{itemize}
The Table \ref{tab:fileReferencing} visualises the changes for two options supported in 0.3.1 version. 
The first the experimental data file \emph{hamren\_2008.csv} is on the same level with 
the \pml file. The second example below shows the situation when the experimental data file is in a folder \emph{data}.
\begin{table}[ht]
\begin{center}
\begin{tabular}{ll}
  \hline
  \pml version $\le$ 0.3 & \pml  0.3.1 \\
  \hline
 \lstset{language=XML}
\begin{lstlisting}
       <ImportData oid="id1">
         <name>hamren_2008</name>
         <url>hamren_2008.csv</url>
         <format>CSV</format>
...
OR
       <ImportData oid="id1">
         <path>data/hamren_2008.csv</path>
         <format>CSV</format>
         <delimiter>COMMA</delimiter>
       </ImportData>
\end{lstlisting}  
\caption{Examples of external file references in previous and current \pml version. 
The first 0.3.1 example show the situation when the experimental data file \emph{hamren\_2008.csv}
is on the same level with the \pml file, the second when it is in a folder \emph{data}.}
\label{tab:fileReferencing}
\end{tabular}
\end{center}
\end{table}
...
%\section{Other changes in 0.3.1}
%
\section{NONMEM dataset referencing added}
\label{sec:NONMEMdataSetReference}
\pml versions $\le$ 0.3 provided the two elements to define external tools:  \xelem{NONMEMdataSet} 
and \xelem{TargetTool}. However only the later had its complementary element \xelem{TargetToolReference}
which can be used in any subsequent simulation or estimation step. 

This has now be fixed by provided the missing \xelem{NONMEMdataSetReference} element. The following 
section provides an use example.

\section{Improved support for simulations}
\label{sec:improvedSimulation}

\subsection{Using NONMEM data file}
\xelem{TrialDesing} provided the only means of defining explicitly the trial design, dosing amount 
and time for simulations. Specifically, the \xelem{Observations} element was provided as the 
option for specifying the observations -- i.e. when a particular variable has to be simulated.

In contrast, NONMEM requires a dummy data file for the simulation tasks which is 
the solely source of trial design, dosing amount and time etc. This option was not supported in the 
versions $\le 0.3$ and this issue is resolved in the current schema.

\begin{itemize}
\item
previously described \xelem{NONMEMdataSetReference} element, see Section 
\ref{sec:NONMEMdataSetReference} is used to reference the NONMEM data file used for a simulation
\item
\xelem{Observations} element is optional.
\end{itemize}

The following code shows how this can be used in a \xelem{SimulationStep}
\lstset{language=XML}
\begin{lstlisting}
\end{lstlisting}  
...
\begin{tabular}{ll}
 ...
 \lstset{language=XML}
\begin{lstlisting}
    <SimulationStep oid="st1">
      
      <ct:VariableAssignment>
        <ct:SymbRef symbIdRef="k1"></ct:SymbRef>
        <ct:Assign>
          <ct:Real>1</ct:Real>
        </ct:Assign>
      </ct:VariableAssignment>
...
\begin{lstlisting}
	...
	<!--  REDUNDANT WHEN NONMEM DATA SET IS USED
        <Observations>
        <Timepoints>
          ...
        </Timepoints>
...
\caption{Comparison of simulation support. In previous versions, the simulated observed 
variables could be specified only explicitly in the \xelem{Observation} element within 
\xelem{SimulationStep}. In 0.3.1 it is also possible using the NONMEM dataset.}
\label{tab:categoriesReferencing}
\end{tabular}
\end{center}
\end{table}


\section{Referencing categorical covariates}
\label{sec:referencingCategories}
Previous \pml versions were unable to define categories using numbers, 
e.g. one could assign strings to the categorical covariate SEX such as F/M but not 1/0.
This is now possible and the following table shows how this works
\begin{table}[ht]
\begin{center}
\begin{tabular}{ll}
  \hline
...
\end{lstlisting}  
\caption{Examples of referencing categories of discrete covariates.}
\label{tab:categoriesReferencing}
\end{tabular}
\end{center}
\end{table}
...
%%%\chapter{Outlook}
%%%
%%%Next public release is due in M42 (August 31, 2014) and should cover number of additional 
%%%language elements, such as discrete data models, Bayesian approach support and more.
