...
knowledge base of			& 							& -- support for distribution functions \\
probability distribution 		& 							&  and quantities \\
...
for covariates, parameters 	&							& ProbOnto for parameters, observations and \\
and observations			&							& covariates \\
\hline
...
When encoding probabilistic uncertainties, the names of a particular distribution and 
their parameters are sufficient to specify a distribution in an unambiguous way as 
in most cases such parameter set is unique. But, because for a number of cases 
two or more parameterisations exist, we need to be careful what 
parameters we use when referring to a distribution, otherwise we might 
and up with a wrong model (see for an example Figure \ref{fig:whatCanGoWrong}).
For this purpose an external standard reference is very useful as it allows to 
considerably reduce the effort of providing the needed distribution in a language 
...
impossible. Other ontologies, we have analysed number of them collected accesible 
via the BioPortal, \cite{noy2009bioportal}, suffer from similar limitations
as they are populated in a similar way.}.  
...
of 90+ distributions but doesn't contain a number of relevant for us cases and/or 
...
\begin{lstlisting}
        <ParameterModel blkId="pm1">
            <IndividualParameter symbId="rabbit">
                <!-- individual def with IIV omitted -->
            </IndividualParameter>
        </ParameterModel>
        
        <ParameterModel blkId="pm2">
            <IndividualParameter symbId="piggy">
                <!-- individual def with IIV omitted -->
            </IndividualParameter>
        </ParameterModel>
\end{lstlisting}
...
\captionsetup[longtable]{skip=0em}
\LTcapwidth=.95\textwidth
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Hierarchical model and Bayesian Inference}\marginpar{{\color{red} \scshape{new}}}
\label{ch:Bayesian}
...
elements, either entirely new, redefined or used in different context.
...
($\beta$ is encoded similarily). $\mu$ is an expression -- note that it could have been
implemented directly in the observation model declaration thanks to ProbOnto's capabilities
to assign expressions to parameters.
\lstset{language=XML}
\begin{lstlisting}
...
\end{lstlisting}
The ............. ({\color{red} \scshape{missing text here}})
\lstset{language=MLX}
\begin{lstlisting}
	lV[j] <- lP[j,1]
	V[j] <- exp(lV[j]) 
...
\begin{lstlisting}
	lVpop <- lPpop[1]
	Vpop <- exp(lVpop)			line 154

	lkpop <- lPpop[2]
	kpop <- exp(lkpop)				line 137
\end{lstlisting}
in PharmML reads
\lstset{language=XML}
\begin{lstlisting}
...
\begin{lstlisting}
	...
	Rinv[1:2,1:2] <- inverse(R[,])			line 196
	TP[1:2,1:2]~dwish(Rinv[ , ], rho) 		line 165
\end{lstlisting}
in PharmML reads
\lstset{language=XML}
\begin{lstlisting}
...
\lstset{language=MLX}
\begin{lstlisting}
	Ttau~dgamma(a_omega_tau, b_omega_tau) 		line 218
\end{lstlisting}
in PharmML reads
\lstset{language=XML}
\begin{lstlisting}
...
\begin{lstlisting}
	# prior on "SIGMA"
	Tpop~dgamma(a_taupop, b_taupop)			line 241
	lTpop <- log(Tpop)
\end{lstlisting}
in PharmML reads
\lstset{language=XML}
...
\subsection{Design without defining arms} 
\label{subsec:noArms}
This option could be used predominantly in single subject scenarios
with the possibility to define dosing, observations, lookup tables etc. without 
the need to define the study arms. The dosing etc. information will still 
be encoded in \xelem{TrialDesign} but without placing them within 
the \xelem{Arms}. 
...
\item 
Defining dosing/observations without arm structure, see section \ref{subsec:noArms},
...
PharmML wrt inline storage of data records and to provide means to encode missing 
                ...
                <mdef:Distribution>
                    <mdef:Piecewise>
                        <Piece xmlns="http://www.pharmml.org/pharmml/0.6/Maths">
                            <ProbOnto>
                                <mdef:DistributionName>Normal2</mdef:DistributionName>
...
\section{Dataset headers {\color{red} \scshape{FINISH}}}
...
Therefore, new \xelem{Header} and \xelem{HeaderRow} elements are introduced
and their use is entirely optional. The first comes with following attributes
\begin{description}
\item[name] which can be any string, see example below. 
\item[headerType] with possible values \{\emph{mainHeader}, \emph{subHeader},
\emph{userDefined}\} 
\item[valueType] optional attribute with options  
\item[rowNumber] specifying the sequence of headers  
\end{description}


rowNumber="1" have to match order="1" etc.




IGNORE=c
Any header lines included in this table can be dropped by specifying IGNORE=@.
When the IGNORE option is omitted, any records containing the character \# in column 1
are ignored.

\section{Covariate model -- extended {\color{red} \scshape{FINISH}}}
FINISH 
FINISH 
FINISH 
FINISH 
...
% \section*{Binomial} 
%\begin{description} 
%\item[Code name] Binomial
%\item[ID] 0000019
%\item[Type] discrete
%\item[Support] $k \in \{0,\dots,n\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] number of trials
%\item[symbol] $n$
%\item[definition] $n \in N_0$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] success probability in each trial
%\item[symbol] $p$
%\item[definition] $p \in [0,1]$
%\end{description}
%\item[PMF] ${n \choose k}\, p^k (1-p)^{n-k}$
%\item[CDF] $I_{1-p}(n - k, 1 + k)$
%  \end{description}
  \section*{Binomial} 
...
\centering
 ...
 \caption{PMF and CDF of the Binomial distribution. R code for CDF: \texttt{Rbeta(1-p, n-k, 1+k)}
 with \emph{Rbeta} function from the \emph{zipfR} R-package.}
...
%\section*{Categorical Nonordered} 
%\begin{description} 
%\item[Code name] CategoricalNonordered
%\item[ID] 0000040
%\item[Type] discrete
%\item[Support] $x \in \{1,\dots,k\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] number of categories
%\item[symbol] $k$
%\item[definition] $k > 0, k \in Z$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] event probabilities
%\item[symbol] $p_1, \ldots, p_k$
%\item[definition] $p_1, \ldots, p_k, \Sigma p_i = 1$
%\end{description}
%\item[PMF] $p(x=i)=p_i$
%\item[CDF] $undefined$
%  \end{description}
%
%    \section*{Categorical Ordered} 
%\begin{description} 
%\item[Code name] CategoricalOrdered
%\item[ID] 0000029
%\item[Type] discrete
%\item[Support] $x \in \{1,\dots,k\}$
%\item[Parameter1]
...
%\begin{description} 
%\item[Code name] GeneralizedPoisson
%\item[ID] 0000179
%\item[Type] discrete
%\item[Support] $k \in \{0,1,2,3,\dots\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] Poisson intensity
%\item[symbol] $\lambda$
%\item[definition] $\lambda > 0$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] dispersion
%\item[symbol] $\delta$
%\item[definition] $\max(-1,-\lambda/4) < \delta < 1$
%\end{description}
%\item[PMF] $\frac{\lambda (\lambda+k\delta)^{k-1}\times e^{-\lambda - k \delta}}{k!}$
%\item[CDF] $-$
%  \end{description}
...
%\section*{Geometric} 
%\begin{description} 
%\item[Code name] Geometric
%\item[ID] 0000158
%\item[Type] discrete
%\item[Support] $k \in \{1,2,3,\dots\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] success probability
%\item[symbol] $p$
%\item[definition] $0< p \leq 1$
%\end{description}
%\item[PMF] $(1 - p)^{k-1}\,p$
%\item[CDF] $1-(1 - p)^k$
%  \end{description}

%\section*{Negative Binomial 1} 
%\begin{description} 
%\item[Code name] NegativeBinomial1
%\item[ID] 0000045
%\item[Type] discrete
%\item[Support] $k \in \{0,1,2,3,\dots\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] number of failures
%\item[symbol] $r$
%\item[definition] $r > 0, r \in N$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] success probability
...
%\item[definition] $p \in [0,1]$
%\end{description}
%\item[PMF] $\binom {k+r-1}k (1-p)^r p^k$
%\item[CDF] $1 - I_{p}(k+1, r) = I_{1-p}(r,k+1)$
%  \end{description}
%  
%  \section*{Negative Binomial 2} 
%\begin{description} 
%\item[Code name] NegativeBinomial2
%\item[ID] 0000056
%\item[Type] discrete
%\item[Support] $k \in \{0,1,2,3,\dots\}$
%\item[Parameter1]
...
%\item[definition] $\tau$
%\end{description}
%\item[PMF] $\frac{\Gamma(k + \frac{1}{\tau})}{k!\; \Gamma(\frac{1}{\tau})} \Big(\frac{1}{1+\tau \lambda} \Big)^{\frac{1}{\tau}} 
%\Big(\frac{\lambda}{\frac{1}{\tau} + \lambda} \Big)^{k}$
%\item[CDF] $-$
%  \end{description}
...
\centering
 ...
 \caption{PMF and PDF of the Negative Binomial distribution, NB1.}
 \label{fig:NB1pmfcdf}
\end{figure}
...
%\item[definition] $\lambda > 0$
%\end{description}
%\item[PMF] $\frac{\lambda^k}{k!}e^{-\lambda}$
%\item[CDF] $\frac{\Gamma(\lfloor k+1 \rfloor,\lambda)}{\lfloor k \rfloor!}$
%  \end{description}
...
\centering
 ...
 \caption{PMF and CDF of the Poisson distribution.}
...
\paragraph{Note} There are surprising many names for the Poisson parameter, such as
...
%\section*{Uniform Discrete 2} 
%\begin{description} 
%\item[Code name] UniformDiscrete2
%\item[ID] 0000184
%\item[Type] discrete
%\item[Support] $x \in \{0,1,2,\dots,n\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] minimum
%\item[symbol] $0$
...
%\item[Parameter2]
% \begin{description}
%\item[Name] number of values (?)
%\item[symbol] $n$
%\item[definition] $?$
%\end{description}
%\item[PMF] $1/(n+1)$
%\item[CDF] $\frac{x+1}{n+1}$
%  \end{description}
%   
%\section*{Uniform Discrete 1} 
%\begin{description} 
%\item[Code name] UniformDiscrete1
%\item[ID] 0000173
%\item[Type] discrete
%\item[Support] $a,b \in \{\dots,-2,-1,0,1,2,3,\dots\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] minimum
%\item[symbol] $a$
%\item[definition] $a \in \{\dots,-2,-1,0,1,2,3,\dots\}$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] maximum
%\item[symbol] $b$
%\item[definition] $b \in \{\dots,-2,-1,0,1,2,3,\dots\}, b \ge a$
%\end{description}
%\item[Parameter3]
% \begin{description}
%\item[Name] number of values (?)
%\item[symbol] $n$
%\item[definition] $n=b-a+1$
%\end{description}
%\item[PMF] $1/n$
%\item[CDF] $\frac{\lfloor k \rfloor -a+1}{n}$
%  \end{description}
  
%\section*{Zero-inflated Poisson} 
%\begin{description} 
%\item[Code name] ZeroInflatedPoisson
%\item[ID] 0000222
%\item[Type] discrete
%\item[Support] $k \in \{0,1,2,3,\dots\}$
%\item[Parameter1]
...
%\item[Parameter2]
% \begin{description}
%\item[Name] probability of extra zeros
%\item[symbol] $\pi$
%\item[definition] $\pi$
%\end{description}
%\item[PMF] $\begin{cases}
...
%  \section*{Multinomial} 
%\begin{description} 
%\item[Code name] Multinomial
%\item[ID] 0000014
%\item[Type] discrete
%\item[Support] $X_i \in \{0,\dots,n\}, \Sigma X_i = n$
%\item[Parameter1]
% \begin{description}
%\item[Name] number of trials
%\item[symbol] $n$
%\item[definition] $n > 0, n \in N$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] event probabilities
...
%\item[definition] $p_1, \ldots, p_k, \Sigma p_i = 1$
%\end{description}
%\item[PMF] $\frac{n!}{x_1!\cdots x_k!} p_1^{x_1} \cdots p_k^{x_k}$
%\item[CDF] $-$
%  \end{description}
\section*{Multinomial} 
...
\centering
 ...
 \caption{PMF and CDF of the Beta distribution.}
...
%\item[Name] shape
%\item[symbol] $\alpha$
%\item[definition] $\alpha > 0$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] shape
%\item[symbol] $\beta$
%\item[definition] $\beta > 0$
%\end{description}
%\item[PDF] $\frac{x^{\alpha-1}(1-x)^{\beta-1}} {Beta(\alpha,\beta)} $
%\item[CDF] $I_x(\alpha,\beta)$
%  \end{description}
  
%\section*{Chi-squared} 
%\begin{description} 
%\item[Code name] ChiSquared
%\item[ID] 0000061
%\item[Type] continuous
...
%\item[symbol] $k$
%\item[definition] $k \in N^\star$
%\end{description}
%\item[PDF] $\frac{1}{2^{\frac{k}{2}}\Gamma\left(\frac{k}{2}\right)}\; x^{\frac{k}{2}-1} e^{-\frac{x}{2}}$
%\item[CDF] $\frac{1}{\Gamma\left(\frac{k}{2}\right)}\; \gamma\left(\frac{k}{2},\,\frac{x}{2}\right)$
%  \end{description}
...
%  \section*{Exponential} 
%\begin{description} 
%\item[Code name] Exponential
%\item[ID] 0000130
%\item[Type] continuous
%\item[Support] $x \in [0,+\infty)$
%\item[Parameter1]
% \begin{description}
%\item[Name] rate or inverse scale
%\item[symbol] $\lambda$
%\item[definition] $\lambda > 0$
%\end{description}
%\item[PDF] $\lambda e^{-\lambda x}$
%\item[CDF] $1 - e^{-\lambda x}$
%  \end{description}
  \section*{Exponential} 
...
%\item[Parameter1]
% \begin{description}
%\item[Name] degree of freedom
%\item[symbol] $d_1$
%\item[definition] $d_1 > 0$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] degree of freedom
%\item[symbol] $d_2$
%\item[definition] $d_2 > 0$
%\end{description}
%\item[PDF] $\frac{\sqrt{\frac{(d_1 x)^{d_1}d_2^{d_2}}
%{(d_1 x+d_2)^{d_1+d_2}}}}
%{x B\left(\frac{d_1}{2},\frac{d_2}{2}\right)}$
%\item[CDF] $I_{\frac{d_1 x}{d_1 x + d_2}} \left(\tfrac{d_1}{2}, \tfrac{d_2}{2} \right)$
%  \end{description}
  \section*{F} 
...
\centering
 ...
 \caption{PDF and CDF of the F distribution.}
 \label{fig:F_pdf_cdf}
\end{figure}
...
%\item[Name] scale
%\item[symbol] $\theta$
%\item[definition] $\theta > 0$
%\end{description}
%\item[PDF] $\frac{1}{\Gamma(k) \theta^k} x^{k \,-\, 1} e^{-\frac{x}{\theta}}$
%\item[CDF] $\frac{1}{\Gamma(k)} \gamma\left(k,\, \frac{x}{\theta}\right)$
%  \end{description}
...
%\section*{Gompertz} 
%\begin{description} 
%\item[Code name] Gompertz
%\item[ID] 0000189
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty)$
%\item[Parameter1]
% \begin{description}
%\item[Name] -
%\item[symbol] $\eta$
%\item[definition] $\eta > 0$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] -
...
%\item[definition] $b > 0$
%\end{description}
%\item[PDF] $b\eta e^{bx}e^{\eta}\exp\left(-\eta e^{bx} \right)$
%\item[CDF] $1-\exp\left(-\eta\left(e^{bx}-1 \right)\right)$
%  \end{description}

%\section*{Gumbel} 
%\begin{description} 
%\item[Code name] Gumbel
%\item[ID] 0000199
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty)$
%\item[Parameter1]
...
%\item[Name] location
%\item[symbol] $\mu$
%\item[definition] $\mu \in  R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] scale
%\item[symbol] $\beta$
%\item[definition] $\beta>0, \beta \in  R$
%\end{description}
%\item[PDF] $\frac{e^{-e^{-\frac{x-\mu}{\beta}}} e^{-\frac{x-\mu}{\beta}}}{\beta}$
%\item[CDF] $e^{-e^{-(x-\mu)/\beta}}$
%  \end{description}

%\section*{Hypergeometric} 
%\begin{description} 
%\item[Code name] Hypergeometric
%\item[ID] 0000209
%\item[Type] continuous
%\item[Support] $k\, \in\, \left\{\max{(0,\, n+K-N)},\, \dots,\, \min{(n,\, K )}\right\}$
%\item[Parameter1]
% \begin{description}
%\item[Name] population size
%\item[symbol] $N$
%\item[definition] $N \in \left\{0,1,2,\dots\right\}$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] number of trials
%\item[symbol] $K$
%\item[definition] $K \in \left\{0,1,2,\dots,N\right\} $
%\end{description}
%\item[Parameter3]
% \begin{description}
%\item[Name] number of successes
%\item[symbol] $n$
%\item[definition] $n \in \left\{0,1,2,\dots,N\right\}$
%\end{description}
%\item[PDF] ${{{K \choose k} {{N-K} \choose {n-k}}}\over {N \choose n}}$
%\item[CDF] $1-{{{n \choose {k+1}}{{N-n} \choose {K-k-1}}}\over {N \choose K}} \,_3F_2\!\!\left[\begin{array}{c}1,\ k+1-K,\ k+1-n \\ k+2,\ N+k+2-K-n\end{array};1\right]$
%  \end{description}
  
%  \section*{Inverse-Gamma} 
%\begin{description} 
%\item[Code name] InverseGamma
%\item[ID] 0000227
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
...
%\item[Name] shape
%\item[symbol] $\alpha$
%\item[definition] $\alpha>0, \alpha \in  R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] scale
...
%\item[definition] $\beta>0, \beta \in  R$
%\end{description}
%\item[PDF] $\frac{\beta^\alpha}{\Gamma(\alpha)} x^{-\alpha - 1} \exp \left(\frac{-\beta}{x}\right)$
%\item[CDF] $\frac{\Gamma(\alpha, \beta/x)}{\Gamma(\alpha)}$
%  \end{description}
  
%  \section*{Laplace} 
%\begin{description} 
%\item[Code name] Laplace
%\item[ID] 0000232
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty)$
%\item[Parameter1]
...
%\item[Name] scale
%\item[symbol] $b$
%\item[definition] $b > 0, b \in R$
%\end{description}
%\item[PDF] $\frac{1}{2\,b} \exp \left(-\frac{|x-\mu|}b \right)$
%\item[CDF] $\begin{cases}
...
%\item[Name] scale
%\item[symbol] $s$
%\item[definition] $s > 0, s \in R$
%\end{description}
%\item[PDF] $\frac{e^{-\frac{x-\mu}{s}}} {s\left(1+e^{-\frac{x-\mu}{s}}\right)^2}$
%\item[CDF] $\frac{1}{1+e^{-\frac{x-\mu}{s}}}$
%  \end{description}
   
%  \section*{Log-Normal 1} 
%\begin{description} 
%\item[Code name] LogNormal1
%\item[ID] 0000237
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
...
%\item[Name] shape
%\item[symbol] $\sigma$
%\item[definition] $\sigma > 0$
%\end{description}
%\item[PDF] $\frac{1}{x\sigma\sqrt{2\pi}}\ e^{-\frac{\left(\ln x-\mu\right)^2}{2\sigma^2}}$
%\item[CDF] $\frac12 + \frac12\,\text{erf}\Big[\frac{\ln x-\mu}{\sqrt{2}\sigma}\Big]$
%  \end{description}
%
%  \section*{Log-Normal 2} 
%\begin{description} 
%\item[Code name] LogNormal2
%\item[ID] 0000242
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
...
% \begin{description}
%\item[Name] shape
%\item[symbol] $variance$
%\item[definition] $variance > 0$
%\end{description}
%\item[PDF] $\frac{1}{x\sigma\sqrt{2\pi}}\ e^{-\frac{\left[\ln (x/m)\right]^2}{2\sigma^2}}$
%\item[CDF] $-$
%  \end{description}
%
%\section*{Log-Normal 3} 
%\begin{description} 
%\item[Code name] LogNormal3
%\item[ID] 0000247
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
% \begin{description}
%\item[Name] median / geometric mean
%\item[symbol] $m$
%\item[definition] $m>0$
%\end{description}
%\item[Parameter2]
...
%  \end{description}
%
%\section*{Log-Normal 4} 
%\begin{description} 
%\item[Code name] LogNormal4
%\item[ID] 0000252
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
...
%\item[Parameter2]
% \begin{description}
%\item[Name] coefficient of variation
%\item[symbol] $cv$
%\item[definition] $cv>0$
%\end{description}
%\item[PDF] $\frac{1}{x\sqrt{\ln(cv^2+1)}\sqrt{2\pi}}\ e^{-\frac{\left[\ln (x/m)\right]^2}{2\ln(cv^2+1)}}$
%\item[CDF] $-$
%  \end{description}
%   
% \section*{Log-Normal 5} 
%\begin{description} 
%\item[Code name] LogNormal5
%\item[ID] 0000257
%\item[Type] continuous
%\item[Support] $x \in (0,+\infty)$
%\item[Parameter1]
...
%\item[Name] precision
%\item[symbol] $tau$
%\item[definition] $tau > 0$
%\end{description}
%\item[PDF] $\sqrt{\frac{\tau}{2 \pi}} \frac{1}{x}e^{-\frac{\tau}{2}(\log x-\mu)^2}$
%\item[CDF] $-$
%  \end{description}
\section*{LogNormal1} 
...
%\item[definition] $\sigma> 0$
%\end{description}
%\item[PDF] $\frac{1}{\sigma \sqrt{2 \pi}}e^{-\frac{(x-\mu)^2}{2\sigma^2}}$
%\item[CDF] $\frac12\left[1 + \text{erf}\left( \frac{x-\mu}{\sigma\sqrt{2}}\right)\right]$
%  \end{description}
%  
%  \section*{Normal 2} 
%\begin{description} 
%\item[Code name] Normal2
%\item[ID] 0000072
%\item[Type] continuous
%\item[Support] $x \in R$
%\item[Parameter1]
...
%\item[Name] variance
%\item[symbol] $var$
%\item[definition] $var>0$
%\end{description}
%\item[PDF] $\frac{1}{\sqrt{var} \sqrt{2 \pi}}e^{-\frac{(x-\mu)^2}{2*var}}$
%\item[CDF] $\frac12\left[1 + \text{erf}\left( \frac{x-\mu}{\sqrt{var}\sqrt{2}}\right)\right]$
%  \end{description}
%  
%\section*{Normal 3} 
%\begin{description} 
%\item[Code name] Normal3
%\item[ID] 0000082
%\item[Type] continuous
%\item[Support] $x \in R$
%\item[Parameter1]
...
%\item[definition] $tau>0$
%\end{description}
%\item[PDF] $\sqrt{\frac{\tau}{2 \pi}} e^{-\frac{\tau}{2}(x-\mu)^2}$
%\item[CDF] $-$
%  \end{description}
...
%\item[definition] $\sigma > 0$
%\end{description}
%\item[PDF] $\frac{x}{\sigma^2} e^{-x^2/2\sigma^2}$
%\item[CDF] $1 - e^{-x^2/2\sigma^2}$
%  \end{description}

%\section*{Student's-T} 
%\begin{description} 
%\item[Code name] StudentT
%\item[ID] 0000144
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty)$
%\item[Parameter1]
...
%\item[definition] $\nu > 0, \nu \in  R$
%\end{description}
%\item[PDF] $\frac{\Gamma \left(\frac{\nu+1}{2} \right)} {\sqrt{\nu\pi}\,\Gamma \left(\frac{\nu}{2} \right)} \left(1+\frac{x^2}{\nu} \right)^{-\frac{\nu+1}{2}}$
%\item[CDF] $\begin{matrix}
...
\centering
 ...
 \caption{PDF and CDF of the StudentT distribution.}
...
%\item[definition] $1$
%\end{description}
%\item[PDF] $e^{-\frac{1}{2} x^2}/\sqrt{2\pi}$
%\item[CDF] $\frac12\left[1 + \text{erf}\left( \frac{x}{\sqrt{2}}\right)\right]$
%  \end{description}
%
%\section*{Standard Uniform} 
%\begin{description} 
%\item[Code name] StandardUniform
%\item[ID] 0000153
%\item[Type] continuous
%\item[Support] $x \in [0,1]$
%\item[Parameter1]
...
%\item[Name] minimum
%\item[symbol] $a$
%\item[definition] $a \in  R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] maximum
%\item[symbol] $b$
%\item[definition] $b \in R, a < b$
%\end{description}
%\item[PDF] $\begin{cases}
...
%\item[Name] scale
%\item[symbol] $\lambda$
%\item[definition] $\lambda\in (0, +\infty)$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] shape
%\item[symbol] $k$
%\item[definition] $k\in (0, +\infty)$
%\end{description}
%\item[PDF] $f(x)=\begin{cases}
%\frac{k}{\lambda}\left(\frac{x}{\lambda}\right)^{k-1}e^{-(x/\lambda)^{k}} & x\geq0\\
%0 & x<0\end{cases}$
%\item[CDF] $\begin{cases}1- e^{-(x/\lambda)^k} & x\geq0\\ 0 & x<0\end{cases}$
%  \end{description}
...
%\section*{Pareto} 
%\begin{description} 
%\item[Code name] Pareto
%\item[ID] 0000098
%\item[Type] continuous
%\item[Support] $x \in [x_m, +\infty)$
%\item[Parameter1]
% \begin{description}
%\item[Name] scale
%\item[symbol] $x_m$
%\item[definition] $x_m > 0, x_m >  R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] shape
%\item[symbol] $\alpha$
%\item[definition] $\alpha > 0, \alpha >  R$
%\end{description}
%\item[PDF] $\frac{\alpha\,x_m^\alpha}{x^{\alpha+1}}\text{ for }x\ge x_m$
%\item[CDF] $1-\left(\frac{x_m}{x}\right)^\alpha \text{ for } x \ge x_m$
%  \end{description}

%\section*{NormalInverseGamma} 
%\begin{description} 
%\item[Code name] Normal- inverse-gamma
%\item[ID] 0000089
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty), \sigma^2 \in (0,+\infty)$
%\item[Parameter1]
% \begin{description}
%\item[Name] location
%\item[symbol] $\mu$
...
%\item[Name] -
%\item[symbol] $\lambda$
%\item[definition] $\lambda > 0, \lambda \in  R$
%\end{description}
%\item[Parameter3]
% \begin{description}
%\item[Name] -
...
%\item[definition] $\beta > 0, \beta \in  R$
%\end{description}
%\item[PDF] $\frac {\sqrt{\lambda}} {\sigma\sqrt{2\pi} }  \frac{\beta^\alpha}{\Gamma(\alpha)} \, \left( \frac{1}{\sigma^2} \right)^{\alpha + 1}   e^{ -\frac { 2\beta + \lambda(x - \mu)^2} {2\sigma^2}  } $
%\item[CDF] $-$
%  \end{description}
  
%\section*{Dirichlet} 
%\begin{description} 
%\item[Code name] Dirichlet
%\item[ID] 0000077
%\item[Type] continuous
%\item[Support] $x_1, \cdots, x_K \quad\text{where}\quad x_i \in [0,1]\quad and \quad\sum_{i=1}^K x_i = 1$
%\item[Parameter1]
% \begin{description}
%\item[Name] number of categories
%\item[symbol] $K$
%\item[definition] $K \geq 2$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] concentration
%\item[symbol] $\alpha_1, \cdots, \alpha_K$
%\item[definition] $\alpha_1, \cdots, \alpha_K, \alpha_i > 0$
%\end{description}
%\item[PDF] $\frac{1}{B(\alpha)} \prod_{i=1}^K x_i^{\alpha_i - 1} \\ \text{where} \quad B(\alpha) = \frac{\prod_{i=1}^K \Gamma(\alpha_i)}{\Gamma\bigl(\sum_{i=1}^K \alpha_i\bigr)} \\ \text{where} \quad \alpha=(\alpha_1,\ldots,\alpha_K)$
%\item[CDF] $-$
%  \end{description}  
      
%\section*{Cauchy} 
%\begin{description} 
%\item[Code name] Cauchy
%\item[ID] 0000050
%\item[Type] continuous
%\item[Support] $x \in (-\infty,+\infty)$
%\item[Parameter1]
...
%\item[Name] location
%\item[symbol] $x_0$
%\item[definition] $x_0 \in  R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] scale
%\item[symbol] $\gamma$
%\item[definition] $\gamma \in  R$
%\end{description}
%\item[PDF] $\frac{1}{\pi\gamma\,\left[1 + \left(\frac{x-x_0}{\gamma}\right)^2\right]}$
%\item[CDF] $-$
%  \end{description}
...
%\begin{description} 
%\item[Code name] InverseWishart
%\item[ID] 0000217
%\item[Type] continuous
%\item[Support] $X(p \times p) - \text{positive-definite matrix}$
%\item[Parameter1]
% \begin{description}
%\item[Name] scale matrix
%\item[symbol] $\Psi$
%\item[definition] $\Psi > 0, \text{positive-definite matrix}$
%\end{description}
%\item[Parameter2]
%\begin{description}
%\item[Name] degrees of freedom
...
%\begin{description} 
%\item[Code name] MultivariateNormal
%\item[ID] 0000024
%\item[Type] continuous
%\item[Support] $x \in \mu + \text{span}(\Sigma) \subseteq  R^k$
%\item[Parameter1]
% \begin{description}
%\item[Name] location
%\item[symbol] $\mu$
%\item[definition] $\mu \in R^k$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] covariance
%\item[symbol] $\Sigma$
%\item[definition] $\Sigma \in R^{k\times k}$
%\end{description}
%\item[PDF] $(2\pi)^{-\frac{k}{2}}|\Sigma|^{-\frac{1}{2}}\, e^{ -\frac{1}{2}(x-\mu)'\Sigma^{-1}(x-\mu) }$
%\item[CDF] $\text{(no analytic expression)}$
%  \end{description}  
...
%\item[Name] scale matrix
%\item[symbol] $V$
%\item[definition] $V > 0, p\times p - \text{positive-definite matrix}$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] degrees of freedom
%\item[symbol] $n$
%\item[definition] $n > p-1$
%\end{description}
%\item[PDF] $\frac{|X|^{\frac{n-p-1}{2}} e^{-\frac{\text{tr}(V^{-1}X)}{2}}}{2^\frac{np}{2}|V|^\frac{n}{2}\Gamma_p(\frac{n}{2})}$
%\item[CDF] $-$
%  \end{description}
...
\smallskip \noindent \hspace{.2cm} \textbf{PDF/PMF} 
\begin{equation*}f(x; \pi, \theta) = \sum_{i=1}^{K} \pi_{i}\; p_i(x; \theta_i)  \end{equation*}
where $p_i(x; \theta_i)$ the PDF/PMF of the $i^{th}$ component with parameters $\theta_i$.
...
%\section*{MultivariateStudentT} 
%\begin{description} 
%\item[Code name] Multivariate (Student) T
%\item[ID] 0000034
%\item[Type] continuous
%\item[Support] $x \in R^p$
%\item[Parameter1]
% \begin{description}
%\item[Name] location
%\item[symbol] $\mu$
%\item[definition] $\mu = [\mu_1, \dots, \mu_p]^T, \mu_i \in R$
%\end{description}
%\item[Parameter2]
% \begin{description}
%\item[Name] scale matrix
%\item[symbol] $\Sigma$
%\item[definition] $\Sigma, \text{ positive-definite real } p\times p \text{ matrix}$
%\end{description}
%\item[Parameter3]
% \begin{description}
%\item[Name] degrees of freedom
...
%\item[definition] $\nu$
%\end{description}
%\item[PDF] $\frac{\Gamma\left[(\nu+p)/2\right]}{\Gamma(\nu/2)\nu^{p/2}\pi^{p/2}\left|\Sigma\right|^{1/2}\left[1+\frac{1}{\nu}(x-\mu)^{\rm T}\Sigma^{-1}(x-\mu)\right]^{(\nu+p)/2}}$
%\item[CDF] $no analytic expression$
%  \end{description}  
