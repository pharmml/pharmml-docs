...
\lstdefinelanguage{XML}
{
  ...
  argument,admNumber,transformId,transformIdRef,metadataFile,fixed,toolName} % list your attributes here
}
...
\input{title_PKtechnicalReport.tex} 
...
been selected as the one which should be supported by PharmML \cite{MLXTRANforMonolix:2014}. 
...
% OUT FOR NOW
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\chapter{PK macros in PharmML} % {\color{red} \scshape{PROMOTE TO CHAPTER}}
%\label{chapter:PKmacros}
%
%\section{MLXTRAN macros}
...
%  \hline
%  \hline
%Component & Macro & Structural arguments & Model arguments \\
%  \hline
%Compartment 			& \textit{compartment}$^{*}$ 	& cmt, amount, concentration & \textbf{V}\\
%Peripheral compartment	& \textit{peripheral}$^{*}$		& amount, concentration 	& \textbf{kij}, \textbf{k$\_$i$\_$j}, \textbf{V} \\
%Effect compartment 		& \textit{effect} 				& cmt, concentration 	&  \textbf{ke0} \\
%Absorption from a depot 	& \textit{depot} 				& type/adm, target 		& \textbf{Tlag}, \textbf{p}, \textbf{Tk0}, \textbf{ka}, \textbf{Ktr}/\textbf{Mtt} \\
...
%					& or \textit{oral}				& \\
%Linear elimination 		& \textit{elimination}$^{**}$	& cmt	 			& \textbf{k}, \textbf{CL} \\
...
%(*) Argument \emph{volume} has been promoted, compared to the original, 
%to the family of \textbf{bold} arguments which means that it can act alone in 
%a macro, see more explanation in text.
%(**) The argument \emph{volume} has  been left out from the 
%\emph{elimination} macro compared to the original description in 
%\cite{MLXTRANforMonolix:2014}. This follows private communication with 
%Lixoft developer teams which decided to remove it in a future version.}
...
elements, will be encoded within the \xelem{Compartment} element. The optional attribute \xatt{argument}
...
\end{itemize}
\item
all \textit{other} arguments (which cannot act alone), e.g. 'cmt' or 'amount' as in the following macro
\lstset{language=NONMEMdataSet}
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Defaults, fixed argument values and exceptions}
\label{subsec:fixedArguments}
\paragraph{Optional arguments}
Some arguments are optional. For example \xatt{adm} is not required if there 
is a single route of administration or the \xatt{amount} argument 
in the \xelem{peripheral} macro. Another example is the macro \xatt{iv()} 
which doesn't require any arguments if the model is one-compartmental. 
See [3] for more details. However, in all following examples we don't use 
defaults for the sake of better understanding of the macro concept.

\paragraph{Fixed and model structure dependent arguments}
All but one macro have a fixed set of arguments. For example the \xatt{compartment} can 
take only these predefined four arguments: \xatt{\{cmt, amount, volume, concentration\}}. 
The exception is the \xatt{peripheral} macro with three fixed
arguments \xatt{\{amount, volume, concentration\}} and a variable one, 
\xatt{k\_ij}/\xatt{k\_i\_j} (and \xatt{k\_ji}/\xatt{k\_j\_i}), where \xatt{i} and
...
symbol \xatt{k21} is not a parameter itself and \xatt{k\_test} serves as the 
transfer parameter \xatt{kij} between compartment 2 and 1, which can be defined 
in the parameter model \xatt{pm1} and can be assigned any value in the 
\xelem{ModellingSteps} section.

\paragraph{Specifying the administration types}
%A very useful by-product of the macro system and its idea of the separation 
%of the input definition and model is that it allows others the numerical
A very useful feature of the macro system is that it allows other then numerical
identifiers for administration types. This also means that the associate dataset 
can contain a meaningful string rather then numbers only, see Table 
\ref{tab:integersOrStrings} for the implantation and 
Table \ref{tab:integersOrStrings_datasets} how an acceding dataset would look like. 
\begin{table}[ht!]
...
\\
    \hline
\end{tabular}
\caption{Using numerical values or strings to identify the administrations.}
\label{tab:integersOrStrings}
\end{table}
...
ID	& TIME  & AMT	 & \textbf{ADM}  &	Y \\
  \hline
1	& 6	    & 10	& \textbf{bolusIV}	 & . \\
1	& 9	    & 20	& \textbf{ORAL}	 & . \\
1	& 18	    & 10	& \textbf{bolusIV}	 & . \\
1	& 33	    & 20	& \textbf{ORAL}	 & . \\
...     &  ...     &  ...     &  ...  & ... \\
1	& 0	    & .	& .	& 0 \\
1	& 12	    & .	& .	& 1.18 \\
...
\caption{MONOLIX datasets for the administration type identified using 
numbers (left) and strings (right) corresponding to the previous 
Table \ref{tab:integersOrStrings}.}
\label{tab:integersOrStrings_datasets}
\end{table}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Macros and target tools}
...
\subsection{Basic translation rules}
...
\paragraph{Models corresponding to ADVAN 1-4 and 10-12}
...
%\newpage
\subsection{Datasets mapping}
The mapping structure as defined in PharmML version 0.6 \cite{Pharmml_06} is used to 
encode the mapping between macros and the MONOLIX dataset, i.e. the mapping between 
...
%\footnote{The attribute name \xatt{admNumber}
%is misleading. See for more details Section \ref{subsec:fixedArguments}}
...
%\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Linking macros and \xelem{TrialDesign}}
...
\begin{itemize}
\item
new value \textit{admType} is added to the \xatt{inputTarget} attribute.
\end{itemize}
...
\end{lstlisting}
and if then the concentration is required, a subsequent assignment for 
$C$ should be provided in MLXTRAN
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Predefined PREDPP models using PK macros and PharmML}
...
To allow for a lossless translation between PREDPP coded models and PK 
macros one needs to established well defined translation rules. To achieve 
this goal and provide help to other translation/converter teams we need to 
analyse the predefined PREDPP routines ADVAN1-4 \& 10-12 and more 
specifically to
\begin{enumerate}
\item
...
\end{lstlisting}
    \\ [+1ex]
      \hline
      \\
\caption{Interpretation and translation of PREDPP models using ADVAN1-4 
\& 10-12 routines, with default TRANS1 parameterization, into PK macros.
Please, note that some attributes are are optional if the context allows it. 
For example \xatt{adm} is not required if there is only one administration route.
See \cite{MLXTRANforMonolix:2014} and Section \ref{subsec:fixedArguments} 
for more details.}
...
\begin{figure}[htbp!]
\centering
 \includegraphics[width=165mm]{pics/AdvanMacrosMaster.pdf}
...
%\paragraph{Version 1}
%PharmML code -- modified from section \ref{subsubsec:ADVAN1}. 
The reparameterization is done in 
the \xelem{ParameterModel}, \xatt{pm1}:
\lstset{language=XML}
\begin{lstlisting}
...
\label{tab:C3Table}
\end{table}
The following code is split in three parts to improve the readability. The first contains
the \xelem{ParameterModel}, the second the \xelem{StructuralModel} with 
\xelem{PKmacros} and the third the \xelem{ModellingSteps}. 
