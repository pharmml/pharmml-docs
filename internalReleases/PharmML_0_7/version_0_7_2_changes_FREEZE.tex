...
\hline
\hline
ProbOnto support, 			& elements implemented as part  	& {\color{red} \scshape{new}} separate ProbOnto schema developed  \\
chapter \ref{ch:071changes} 	& of PharmML schema			& -- allows flexible extending of the distribution  \\
						&							& collection \\
see appendix \ref{ch:probontoAppendix}	& $\sim$55 distribution available	& -- 64 distribution available \\
\hline
Mathematical expressions,	& \xelem{Equation} elements required &  \xelem{Equation} element removed  \\
chapter \ref{ch:071changes}	&							& \\
\hline
Dataset mapping, see		& \xatt{columnType} attribute is  	& \xatt{columnType} attribute is optional \\
chapter \ref{ch:071changes}	& mandatory					&\\
\hline
Defining sequences, see 		& two options supported 			& {\color{red} \scshape{new}} option Begin:StepNumber:End \\
chapter \ref{ch:071changes}	& -- Begin:StepSize:End and 		& -- \xelem{Repetitions} renamed to \xelem{StepNumber} \\
...
\section*{Major changes/extensions in version 0.7.2}
...
\hline
\hline
\pml element 				&  version $\le$ 0.7.1 			& version 0.7.2 \\
or modelling aspect 			& 							& \\
\hline
\hline
Optimal design tasks, 		& Not supported				& {\color{red} \scshape{new}} Evaluation/optimisation tasks   \\
section \ref{sec:newDesignTasks}	& 						& supported \\
\hline
ProbOnto -- general,		& 64 distributions/parameterisations & -- 71 distributions/parameterisations  \\
section \ref{sec:POextentions} 	& No relationships or properties	& -- more then 30 relationships and   \\
						& 		 					& -- 40 re-parameterisation formulas \\
Count data models,			& 7 models covered				& Additional 5 models: new parameterisations \\
section \ref{subsec:countModels}	&						& for generalised Poisson, negative binomial \\
						&							& and {\color{red} \scshape{new}} Double Poisson \\
\hline
Vectors, section \ref{sec:otherChanges072} & mandatory elements & Can be empty assumed it is defined using \\
						&							& attributes \xatt{default} and \xatt{length} \\
\hline
Matrix operators			& Available \xatt{inverse}, \xatt{trace}& {\color{red} \scshape{new}}  \xatt{determinant} added \\
section \ref{sec:otherChanges072} &						& \\
\hline
Dataset's \xatt{columnType}	& Number of values allowed since 0.7 & {\color{red} \scshape{new}} \emph{varParameter\_var}, \emph{varParameter\_stdev},  \\
section \ref{sec:otherChanges072}	& for SO 					& \emph{varParameter\_cov}, \emph{varParameter\_corr} \\
						&							& to be used in SO tables\\
\hline
Dosing target type			& \xatt{inputTarget} mandatory 	& \xatt{inputTarget} attribute is optional \\
specification				&							& \\
\hline
\caption{Overview of major differences between versions 0.7.1 and 0.7.2.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{input/appendix_GNB}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{input/appendix_NB}
