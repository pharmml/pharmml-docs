...
\hline
Annotation and encoding of 	&  	--						& \emph{ProbOnto} - ontology and knowledge  \\
statistical models			&							& base of probability distribution 	\\
...
observations model  		&							& can be used with either \uncertml or \\
			 			&							& ProbOnto for parameters, observations \\
						&							& and covariates \\
\hline
Individual parameter model	& 3 types supported: 			& 4 types supported now \\
						& -- \emph{Equation}-type		& -- {\color{red} \scshape{modified}} \emph{Structured}-type \\
...
Population parameter		& \xelem{SimpleParameter} used	&{\color{red} \scshape{new}} \xelem{PopulationParameter} element \\	
						& with no distribution support		& replaces \xelem{SimpleParameter} element \\
\hline
...
Matrix/Vector operators		& not supported				& {\color{red} \scshape{new}} \xelem{MatrixUniOp} element with \\
						&							& values \emph{inverse, trace and transpose} \\
\hline
Transformation element 		& supported 					& new structure with attribute \xatt{type} \\
						&							& to be assigned values such as \emph{log}, \emph{logit} etc. \\
...
Population parameter		& not supported				& {\color{red} \scshape{new}} \xelem{PopulationParameter} with \\
 						& 							&  \emph{Distribution}-type or \emph{Equation}-type \\
\hline
\hline
  \multicolumn{3}{c}{\textit{TRIAL DESIGN}}  \\
\hline
\hline
Structure 					& CDISC based				& redesigned based on WP3 design proposal \\
						&							& -- all design elements in \xelem{TrialDesign}  \\
...
\hline
\hline
Interval					& not available					& {\color{red} \scshape{new}} \xelem{Interval} with left/right- \\
...
and parameters, sec. \ref{sec:BoxCoxTrafo}	&				& \xelem{Parameter} child element for \emph{lambda} \\
						&							& parameter \\
...
%\item
%All mathematical functions and quantities are available in Latex and partially R-code
%is provided.
%\end{itemize}
%\item
...
%of many distributions. The majority of parameter names is identical to those used 
%in UncertML. This is to keep the nomenclature consistent within PharmML and MDL, where
%the UncertML vocabulary is mostly used. In tables \ref{figTable:univariates} -- 
%\ref{figTable:multivariatesCodes} and \ref{figTable:mixtures} 
%we have compiled the \emph{code names}. 
%
%\begin{example}
...
%\begin{itemize}
%\item
%\xelem{ProbOnto} tag with the \xatt{name} attribute for the distribution code names, tables 
...
%a tool designer which allows it to be used across the target tools and languages. 
...
%\end{lstlisting}
%
%\subsubsection{Support in 0.7 version}
