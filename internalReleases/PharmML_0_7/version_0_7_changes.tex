...
we have compiled the \emph{code names}. Multiple examples in this and next sections/chapters 
show how to apply the above rules in implementing distributions.
...
\ref{figTable:univariatesCodes} and \ref{figTable:multivariatesCodes}. Its main feature is that 
...
independent from PharmML. This allows it to be used across the target 
tools and languages. It doesn't enforce a certain way to implement it from
a tool designer.
...
and/or alternative parameterisations (37 out of about 55). 
...
\smallskip \noindent \hspace{.2cm} \textbf{CDF in R}  
\begin{verbatim}1/2 + 1/2*erf( (log(x)-mu) / (sqrt(2*v)))\end{verbatim}
