...
annotation and encoding of 	& distributions					& children \xelem{ProbOnto} and \xelem{UncertML} \\
statistical models,			& 							& {\color{red} \scshape{new}} \emph{ProbOnto} - ontology and knowledge  \\
...
\hline
\hline
Covariates, parameters and 	& not available					& {\color{red} \scshape{new}} \emph{Distribution}-type model\\
observations model,   		&							& can be used with either \uncertml or \\
chapter \ref{ch:Models}		&							& ProbOnto for parameters, observations \\
						&							& and covariates \\
\hline
Individual parameter model,	& 3 types supported: 			& 4 types supported now \\
...
\hline
Observation model			& Gaussian and \emph{Equation}-type	& \\
...
section \ref{sec:condDistros}	& and distribution of covariates 	& wrt covariates or design elements  \\
						& supported					& \\
\hline
Matrix/Vector operators,		& not supported				& {\color{red} \scshape{new}} \xelem{MatrixUniOp} element with \\
...
\hline
Covariate model,			& not supported				& \xelem{CovariateModel} has an additional option \\
section \ref{sec:minorChanges}&							& for creating new covariates out of exiting ones \\
\hline
