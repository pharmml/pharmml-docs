...
%\end{lstlisting}
%\item 
%assignment of probabilities in categorical distributions, see for instance
%\cite{Swat:2015b} where an example explains that the order categories in 
%\xelem{ListOfCategories} and probability parameters in \xelem{Parameter} 
%must be preserved.
%
%\end{itemize}
%
%If we follow the rules -- it works.
...
\item
Parameters or variables assigned within the conditional statement 
cannot be assigned elsewhere unless the assignment 
happens at the same time with the declaration, e.g.
\lstset{language=XML}
\begin{lstlisting}
            <PopulationParameter symbId="k">
                <ct:Assign>
...
\end{lstlisting}
If another assignment is defined in a conditional statement elsewhere in the model 
it has precedence over this assignment.
\item 
...
\hline
\hline
  \multicolumn{2}{c}{Option 1: Data stored together with parameter} \\
\hline
\lstset{language=XML}
\begin{lstlisting}
    <ParameterModel blkId="PM1">
