...
500 probability distributions is an invaluable resource of parametric distributions. 
Unfortunately, it comes with univariate cases only, without references or parameter
names making its use in our context impracticable\footnote{Another case is Distributome, 
...
%LogNormal distribution (two reals numbers): 
%geometric mean (exponential of the mean in log-space) and geometric standard deviation (exponential, stricly superior to 1, of the standard deviation in log-space). 
...
\xatt{Geometric} 			& \emph{p}		& \xatt{probability}		& --			& --	\\
\xatt{Hypergeometric} 		& \emph{N}		& \xatt{populationSize}	& \emph{K}	& \xatt{numberOfSuccesses} \\[-0.5ex]
						&				&					&\emph{n}	& \xatt{numberOfTrials} \\
\xatt{InverseBinomial} 		& $k$ 			& \xatt{k}				& $p$		& \xatt{p}	\\
...
\xatt{ZeroInflatedNegativeBinomial} 	& $\lambda$		& \xatt{rate}			& $\tau$ 		& \xatt{overdispersion}	\\
...
\xatt{BirnbaumSaunders} 		& $\alpha$		& \xatt{scale}			& $\gamma$	& \xatt{shape}	\\
...
\xatt{Gamma}				& $k$			& \xatt{shape}			& $\theta$	& \xatt{scale} 	\\
...
\xatt{InverseGaussian}		& $\lambda$		& \xatt{shape}			& $\mu$		& \xatt{mean}		\\
...
\xatt{LogLogistic}			& $\alpha$		& \xatt{scale}			& $\beta$		& \xatt{shape}		\\
...
\xatt{Nakagami}			& $m$			& \xatt{shape}			& $\Omega$ 	& \xatt{spread}		\\
...
\xatt{Triangular}			& $a$			& \xatt{lowerLimit}		& $b$ 		& \xatt{upperLimit}	\\[-0.5ex]
						&				&					& $c$		& \xatt{shape} \\
\xatt{TruncatedNormal}		& $\mu$			& \xatt{mean}			& $\sigma$ 	& \xatt{stdev}	\\[-0.5ex]
