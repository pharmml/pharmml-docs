...
Negative Binomial 1		& \emph{r, p}		&	y\footnote{Negative Binomial is defined in UncertML but using a different distribution/support interpretation, see section \ref{app:sec:NB1discussion}.}	&	y	& --  &  -- \\
...
Weibull 2				& $\lambda, v$		& --	& y	& --	&  -- \\
   \hline 
\caption{{\color{red} \scshape{0.7.1 update}} Univariate distributions supported in ProbOnto with overview of tool support. 
...
\xatt{InverseBinomial1} 		& $k$ 			& \xatt{k}				& $p$		& \xatt{p}	\\
...
\xatt{Weibull2}				& $\lambda$		& \xatt{lambda}			& \emph{v}	& \xatt{shape}		\\
   \hline 
\caption{{\color{red} \scshape{0.7.1 update}} Code names for distribution and parameter names of the univariate distributions.}
...
\xatt{Wishart2} 				& $R$		& \xatt{inverseScaleMatrix} & $k$		& \xatt{degreesOfFreedom} \\
   \hline
\caption{{\color{red} \scshape{0.7.1 update}} Code names for the multivariate distributions and their parameters.}
...
\xatt{MixtureDistribution1} & $\pi_1, \ldots, \pi_k$ & \xatt{weight} &	y \\
  \hline
   \hline
\caption{{\color{red} \scshape{0.7.1 update}} Mixture distribution -  so far only for univariate distributions.}
