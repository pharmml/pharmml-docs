...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[RO]{PharmML 0.8-0.8.1}
...
\lstdefinelanguage{XML}
{
  ...
  toolName,dataCode,missingDataType,regressor,fixed,transformation,transform,symbol,
  referenceLevel,ref} % list your attributes here
}
...
This document describes extensions and changes in \pml between versions 0.7.3 \& 0.8 
and 0.8 \& 0.8.1. The evolution of \pml since the project start	 are visualised in Figure \ref{fig:history}.

\begin{figure}[ht!]
\centering
  \includegraphics[width=160mm]{pics/Timeline-081.pdf}
 \caption{PharmML release history with major extensions shown of each version.}
 \label{fig:history}
\end{figure}

\section{Summary of changes/extensions in 0.8}
The following table summarises the major changes in version 0.8 compared to 0.7.3 
...
Section \ref{sec:assignments}	& 					& in covariate, parameter, structural and \\
						&					& observation models \\
\hline
Independent variable   		& only one allowed		& multiple \xelem{IndependentVariable} elements \\
Section \ref{subsec:greco}	& 					& allowed \\
\hline
...
\hline
Parameter, (random) variables	& \xatt{symbId} only 		& additionally also \xatt{symbIdRef} available when \\
						&					& referencing required \\
\hline
Parameters, Section \ref{sec:simpleParameter} & \xelem{SimpleParameter} not & restored and available as \xelem{Parameter}\\
						& available in v0.7-0.7.3	&  \\
\hline
Nested piecewise			& Simple piecewise 		& supported \\
...
Section \ref{subsec:TTEmodel} & fcts formulas required	& \\
\hline
Empirical distribution		& not supported		& {\color{red} \scshape{new}}  \xatt{RandomSample}, \xatt{SystematicSample} \\ 
Section \ref{sec:nonParamDistribs}	& 				& \xatt{UnknownSample} and \xatt{weight} parameter accessible \\
						&					&  via ProbOnto\\
\hline
Random realisations			& not supported		& {\color{red} \scshape{new}} \xelem{Realisation} for all distributions \\
Section \ref{sec:randRealisations}	 &				& featured in ProbOnto  \\
\hline
...
Section \ref{sec:statsOps}	&					& \xatt{correlation}, \xatt{decile}, \xatt{geometricMean}, \xatt{kurtosis}, \\
						&					& \xatt{mean}, \xatt{median}, \xatt{mode}, \xatt{moment}, \xatt{percentile}, \\
						&					& \xatt{quantile}, \xatt{quartile}, \xatt{range}, \xatt{skewness}, \\
						&					& \xatt{standardDeviation}, \xatt{variance}\\
\hline
...
\hline
Dataset  					& single ignore character	& conditional ignoring statements \\
Section \ref{subsec:condIgnore}	& declaration only	&  \\
\hline
...
\section{Summary of changes/extensions in 0.8.1}
...
\hline
\hline
\pml element 				&  version $\le$ 0.8 	& version 0.8.1 \\
or modelling aspect 			& 					& \\
\hline
Setting and output files		& no support			& {\color{red} \scshape{new}} \xelem{SoftwareSettings} added \\	
\ref{sec:filesSupport}		&					& {\color{red} \scshape{new}} \xelem{OutputFile} added \\
\hline
Dataset definition 				& missing or redundant  & \xatt{columnType} with new values \\
section \ref{subsec:columnType}	& values				&  \emph{varLevel}, \emph{variance}, \emph{stdev}, \emph{mode}, \emph{median} {\color{red} \scshape{new}}  \\
							&					& -- removed \emph{varParameter\_corr}, \emph{varParameter\_cov}, \\
						&					& \emph{varParameter\_stdev}, \emph{varParameter\_var} \\
						&					& -- multiple values allowed \\
-- for use in SO only			& not supported 		& {\color{red} \scshape{new}} \xatt{level} attribute \\
see \cite{SO:2016b}	&					& \\
\hline
Optimal Design 			& missing stage support	& {\color{red} \scshape{new}} \xelem{StageDefinition} element with  \\
Section \ref{sec:OEDextentions} &					& \xelem{Stage} child element \\
						& missing params to estimate 		& {\color{red} \scshape{new}} \xelem{ParametersToEstimate} \\
						& support	in OD		& \\
						& FIM type missing		& -- {\color{red} \scshape{new}} \xatt{type} with values \{B, I, P\} \\ 
						&					& -- \xelem{Matrix} support removed \\
\hline
Variables					& attribute mandatory	& \xatt{symbolType} attribute is optional \\
section \ref{sec:schemaChanges}	&					& \\
\hline
Integer declaration			& types used: \xatt{int}, \xatt{integer} or  & \xatt{int} only \\
section \ref{sec:schemaChanges}	& \xatt{positiveInteger}	& \\
\hline
Piecewise declaration		& occurring in many namespaces & declared in \xatt{math} namespace 	only \\
section \ref{sec:schemaChanges}	&					& \\
\hline
Conditional statements		& ODEs not supported	& ODEs allowed \\
Section \ref{sec:ODEinCS}	& 					& \\
\hline
Functions					& not supported	& {\color{red} \scshape{new}} \xatt{gamma} \\
Section \ref{sec:minors}		& 				& \\
\hline
\caption{Overview of major differences between versions 0.8.1 and 0.8}
\label{figTable:overviewTable}
\vspace{-2em}
\end{longtable}
