...
\begin{itemize}
\item 
more then 80 examples provided with the release and
\item 
14 IOG use cases\footnote{\url{http://sourceforge.net/projects/ddmore/files/install/SEE/Demonstrator-1.2.0/resources/converter-systemtest-1.3.0-results.zip/download}} publicly released with the Product 4.1.
\end{itemize}
...
%\begin{figure}[ht!]
%\centering
%  \includegraphics[width=100mm]{pics/ProbOnto_tree}
% \caption{ProbOnto\_tree.}
...
and $POP\_V = POP\_K\_V[2]$ -- this part is skipped, 
\marginpar{FINISH}
...
\centering
 ...
 \caption{An example of a conditional transition matrix.}
...
\end{align}
or using a conditional transition matrix, Figure \ref{fig:markovMatrixCond}.
See also section \ref{sec:markovModels} for use examples.
...
\begin{itemize}
\item 
\xatt{X} can be either a variable reference, real vector, vector/matrix (if a column is
...
\xatt{max} operator to find the maximum value in a vector of numbers
and symbol references to assign it to a new parameter, e.g. 
Amax = max(1, a2, 3, a4, fifthElement, $\dots$).
\lstset{language=XML}
\begin{lstlisting}
...
\centering
 ...
 \label{fig:quantiles}
\end{figure}
The dataset \xatt{scores = [78, 84, 83, 80, 94, 90, 81, 79, ..., 88, 80, 82, 81, 92, 89]} 
can be imagined being mapped from an external file or 
            ...
            <PopulationParameter symbId="scores">
                <Distribution>
                    <po:ProbOnto name="RandomSample">
                        <ds:DataSet>
                            <ds:Table>
                                <!-- 78, 84, 83, 80, 94, 90, 81, 79, ... 88, 80, 82, 81, 92, 89 -->
                                <ds:Row>
