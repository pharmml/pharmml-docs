...
\label{subsec:greco}
The introduction of assignment statements opens new opportunities 
to encode models relevant e.g. for studying of drug-drug interactions.
While the majority of such models was implementable in previous versions,
the so-called \emph{Greco-model}, \cite{Greco:1990qv},
...
the effect cannot be expressed directly in terms of $d_1$ and $d_2$ as a function 
$E = f(d_1,d_2,\dots)$. The full PharmML code for this model
is provided with this release in the example folder \emph{others}.

It is an interesting model for another reason as well. It is one which requires the 
...
flexibility to the language. So far only piecewise statements were supported
...
The extend to which various target tools cover conditional assignments differs
and posses a problem for the interoperability. Below we summarise
...
translation team, in principle, thanks to the Pascal language interface, 
virtually any nested conditional statement can be implemented for the 
execution in winBUGS. 
...
\end{algorithmic}
\bigskip
The following definition of the conditional statement has been adapted from the 
STAN specification v2.9.0, \cite{stan-manual:2015b}: \\
...
declaration of a population parameter
\lstset{language=XML}
\begin{lstlisting}
...
\centering
 ...
 Because any domain declaration is allowed in PharmML and no automatic validity 
 check is performed, such declarations are ultimately the responsibility of the modeller.}
 \label{fig:CLCR_AGE}
\end{figure}
...
\end{lstlisting}
Note that the attribute \xatt{symbIdRef}, used to reference the individual parameters
in each statement, was not available for parameters or variables previously.
...
The probability functions turn out to be very useful also in the encoding 
of time-to-event data. While so far encoding of hazard or survival functions
formulas was the only option to defined such models, the function elements offer a quick 
...
\\
\hline
\caption{Two ways to encode samples. 
...
$n$ is equal the row number, see Table \ref{tab:randomSample_withoutWeight}.
...
... \\
\hline
\caption{A dataset represented using equiprobable samples. Because no \xatt{weight} parameter
is defined, the assumption is that all values are equiprobable with $p=1/n$ where
$n$ is equal the row number.}
...
\begin{figure}[ht!]
\centering
  \includegraphics[width=160mm]{pics/Samples_modelDef_trialDesign}
...
This is an example for a model with two parameters $POP\_K$ and 
$POP\_K$ drawn from a random sample with probability $p$, see 
Table \ref{tab:randomSampleExternal} and Figure \ref{fig:Mapping_withWeights} for a graphical 
explanation,
...
\item 
\xatt{weight} parameter \emph{p} declared as \xelem{PopulationParameter} applies to each row.
\end{itemize}
...
external dataset, is very useful also in the SO context, \cite{SO:2016b}. 
...
univariate distribution and is based on analogue feature	 in UncertML, 
...
on the sampled covariate value \emph{z}
\lstset{language=MLX}
\begin{lstlisting}
            <Covariate symbId="cz">
                <Categorical>
...
Stochastic (aka transition) matrix is used to describe the transitions in a Markov 
chain\footnote{\url{https://en.wikipedia.org/wiki/Stochastic_matrix}}. A stochastic matrix can be 
characterised further by an optional attribute 
\begin{itemize}
\item
...
\centering
 ...
 states cat1, cat2, and cat3. (Right) The left stochastic transition matrix.}
...
Figure \ref{fig:markovMatrixCond}. See also section \ref{sec:markovModels} for complete examples.
...
\subsubsection*{Example}
An example of the application of N-ary operators is shown below using the
\xatt{max} operator acting on a vector of arguments, e.g. Amax = max(1, a2, 3, a4, fifthElement, $\dots$).
\lstset{language=XML}
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The return of the basic parameter}
...
restored under the name \xelem{Parameter}. It turned out that it is useful in 
QSP and SB modelling where distinction between individual or population 
parameters is not possible or required.

Because version 0.7.3 was not in use so far neither in the IOG use case nor 
tool converters, the restoration of the simple \xelem{Parameter} shouldn't 
cause any confusions apart from the required renaming. Below we provide a short overview 
of parameters as used in last three major versions
\begin{itemize}
\item 
version 0.6.1
\begin{itemize}
\item 
\xelem{SimpleParameter} -- assignment 
\item 
\xelem{IndividualParameter}  -- assignment, structured models with linear and 
nonlinear covariate models, fixed and random effects and variability level reference
\end{itemize}
\item 
version 0.7.3
\begin{itemize}
\item 
\xelem{IndividualParameter} -- assignment, structured models with linear and 
nonlinear covariate models, fixed and random effects; distribution model type
and variability level reference
\item 
\xelem{PopulationParameter} -- assignment or distribution model type and variability level reference
\end{itemize}
\item 
version 0.8
\begin{itemize}
\item 
\xelem{Parameter} -- assignment 
\item 
\xelem{IndividualParameter} -- assignment, structured models with linear and 
nonlinear covariate models, fixed and random effects; distribution model type
and variability level reference
...
\end{itemize}
\end{itemize}
In the majority of the use cases the \xelem{SimpleParameter} has 
been replaced by the \xelem{PopulationParameter} except models where 
\xelem{Parameter} is better suited. See for application examples such as 
models on drug-drug interaction, \emph{../others/GrecoTung\_drugDrugInteraction.xml},
and a PBPK model, \emph{../others/BradshawPierce\_2007.xml}. The last one 
is an update of a v0.4 model converted by the Cyprotex Model Writer.
